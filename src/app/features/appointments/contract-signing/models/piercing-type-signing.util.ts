...
/**
 * Inserta o reemplaza la respuesta de tipo de piercing (pregunta fija) en el payload.
 * Envía el valor **canónico** de `options_json` (Helix, Lobulos, Surface…),
 * no la etiqueta de UI («Lóbulo», «Pezón»…): el backend valida pertenencia exacta.
 */
export function withPiercingTypeSurveyAnswer(
  ...
  const resolved =
    resolvePiercingTypeCanonical(piercingTypeCanonical)?.trim() ||
    piercingTypeCanonical.trim();
  ...
  const answer: SurveyAnswerPayload = { question_id: qid, text: resolved };
  return {
